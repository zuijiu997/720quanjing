...
if($order['status']==0 && $opstate==0){
        $extcredits = $ovalue*$ec_ratio;
    if($has_c)
        C::t('#youka_pay#youka_pay_log')->updatecard_by_orderid($orderid,$ovalue,$extcredits,$sysorderid,$completiontime);
    else{
        DB::query("update ".DB::table('youka_pay_log')." set status=1,price='{$ovalue}',extcredits='{$extcredits}',sysorderid='".$sysorderid."',completiontime='".$completiontime."' WHERE orderid='$orderid' limit 1");
    }
    updatemembercount($order['uid'], array($order['credit_type'] => $extcredits), 1, 'AFD', $order['uid']);
}
if($opstate>1){
    if($has_c)
        C::t('#youka_pay#youka_pay_log')->updatestatus_by_orderid($orderid,$opstate);
    else{
        DB::query("update ".DB::table('youka_pay_log')." set status=$opstate WHERE orderid='$orderid' limit 1");
    }
}
