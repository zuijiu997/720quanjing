	...
	<tr><td class="plc plm">
			...
			<!--{if $post['invisible'] == 0}-->
				<div id="p_btn" class="mtw mbm hm cl">
					...
					<!--{if $_G['group']['raterange'] && $post['authorid']}-->
                        ...
                        <div class="deanicon_bottom"><span class="deansc">{lang rate}</span></div>
                        </a>
					<!--{/if}-->
					<!--{if !$post['anonymous'] && $post['first'] && helper_access::check_module('follow')}-->
                        ...
                        <div class="deanicon_bottom"><span class="deansc">{lang thread_realy}</span><!--{if $_G['forum_thread']['relay']}--><span id="relaynumber" >{$_G['forum_thread']['relay']}</span><!--{/if}--></div>
                        
                        </a>
					<!--{/if}-->
					<!--{if $post['first'] && helper_access::check_module('share')}-->
						<a class="sharep" href="home.php?mod=spacecp&ac=share&type=thread&id=$_G[tid]" onclick="showWindow('sharethread', this.href, 'get', 0);" title="{lang share_digest}"><img src="$_G['style'][styleimgdir]/viewthread/share.png" alt="{lang thread_share}" /><div class="deanicon_bottom"><span class="deansc">{lang thread_share}<!--{if $_G['forum_thread']['sharetimes']}--></span><span id="sharenumber">{$_G['forum_thread']['sharetimes']}</span><!--{/if}--></div></a>
					<!--{/if}-->
					...
					<!--{if !$_G['forum']['disablecollect'] && helper_access::check_module('collection')}-->
                        ...
                        <div class="deanicon_bottom"><span class="deansc">{lang collection}</span><span id="collectionnumber"{if !$post['releatcollectionnum']} {/if}>{$post['releatcollectionnum']}</span></div>
                        
                        </a>
					<!--{/if}-->
					<!--{if ($_G['group']['allowrecommend'] || !$_G['uid']) && $_G['setting']['recommendthread']['status']}-->
						<!--{if !empty($_G['setting']['recommendthread']['addtext'])}-->
						<a id="recommend_add" href="forum.php?mod=misc&action=recommend&do=add&tid=$_G[tid]&hash={FORMHASH}" {if $_G['uid']}onclick="ajaxmenu(this, 3000, 1, 0, '43', 'recommendupdate({$_G['group']['allowrecommend']})');return false;"{else} onclick="showWindow('login', this.href)"{/if} onmouseover="this.title = $('recommendv_add').innerHTML + ' {lang activity_member_unit}$_G[setting][recommendthread][addtext]'" title="{lang maketoponce}"><img src="$_G['style'][styleimgdir]/viewthread/check.png" alt="$_G['setting']['recommendthread'][addtext]" /><div class="deanicon_bottom"><span class="deansc">$_G['setting']['recommendthread'][addtext]</span><span id="recommendv_add"{if !$_G['forum_thread']['recommend_add']} {/if}>$_G[forum_thread][recommend_add]</span></div></a>
						<!--{/if}-->
						<!--{if !empty($_G['setting']['recommendthread']['subtracttext'])}-->
						<a id="recommend_subtract" href="forum.php?mod=misc&action=recommend&do=subtract&tid=$_G[tid]&hash={FORMHASH}" {if $_G['uid']}onclick="ajaxmenu(this, 3000, 1, 0, '43', 'recommendupdate(-{$_G['group']['allowrecommend']})');return false;"{else} onclick="showWindow('login', this.href)"{/if} onmouseover="this.title = $('recommendv_subtract').innerHTML + ' {lang activity_member_unit}$_G[setting][recommendthread][subtracttext]'" title="{lang makebottomonce}"><img src="$_G['style'][styleimgdir]/viewthread/close.png" alt="$_G['setting']['recommendthread'][subtracttext]" /><div class="deanicon_bottom"><span class="deansc">$_G['setting']['recommendthread'][subtracttext]</span><span id="recommendv_subtract"{if !$_G['forum_thread']['recommend_sub']} {/if}>$_G[forum_thread][recommend_sub]</span></div></a>
						<!--{/if}-->
					<!--{/if}-->
		...
		<!--{if $post['relateitem']}-->
			<div class="deanmean mtw mbw">
				<h3 class="pbm mbm bbda deanrelatedthreads"><span>{lang related_thread}</span></h3>
				<ul class="xl xl2 cl deanthreadslists">
