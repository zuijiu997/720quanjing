					...
					<div class="deanbiaotou">
                        ...
                        <div class="y deanspace">
                        	<span id="clearstickthread" style="display: none;">
                                <a href="javascript:;" onclick="clearStickThread()" class="dean_showdisplays" title="{lang showdisplayorder}">{lang showdisplayorder}</a>
                            </span>
                            ...
                            <!--{else}-->
                            <a{if empty($_G['cookie']['forumdefstyle'])} href="forum.php?mod=forumdisplay&fid=$_G[fid]&forumdefstyle=yes" class="deanwfpower chked"{else} href="forum.php?mod=forumdisplay&fid=$_G[fid]&forumdefstyle=no" class="deanwfpower unchk"{/if} title="{lang view_thread_imagemode}{lang view_thread}">{lang view_thread_imagemode}</a>
                            <!--{/if}-->
                        </div>
					</div>
			...
			<table summary="forum_$_G[fid]" cellspacing="0" cellpadding="0" id="threadlisttableid">
                <!--{if (!$simplestyle || !$_G['forum']['allowside'] && $page == 1) && !empty($announcement)}-->
						<div class="deanancmtlist">
							...
							<div class="deanannouncement">{lang announcement}: </div>
							...
							<div class="deananmtauthor"><a href="home.php?mod=space&uid=$announcement[authorid]" c="1">$announcement[author]</a></div>
							<div class="deananmtdate">	<em>$announcement[starttime]</em></div>
                            <div class="clear"></div>
						</div>
				<!--{/if}-->
				...
				<!--{if $_G['forum_threadcount']}-->
							...
							<tbody id="$thread[id]">
								<tr class="deanactions fadeIn">
									<td class="icn">
                                        ...
                                        <a class="deanthreads_author" href="home.php?mod=space&uid=$thread[authorid]" c="1">$thread[author]</a>
                                    </td>
									...
									<th class="$thread[folder] forumtit">
										...
										<!--{elseif in_array($thread['displayorder'], array(1, 2, 3, 4))}-->
											<img src="$_G['style'][styleimgdir]/forumlist/pin_$thread[displayorder].png" alt="$_G[setting][threadsticky][3-$thread[displayorder]]" />
										<!--{else}-->
                                        ...
                                        <div class="deanthreads_summary">
                                        	<!--{eval include 'template/zvis_source_170511/php/summary.php'}-->
                                                <!--{echo cutstr($summary_echo,200)}-->
                                        </div>

                                        <div class="deanforumlist_pics">
                                        	<!--{eval include 'template/zvis_source_170511/php/innerpics.php'}-->
											<!--{loop $yuerbaike $hypic}-->
                                        	<a href="forum.php?mod=viewthread&tid=$thread[tid]" target="_blank">
                                            	<img src="data/attachment/forum/$hypic[attachment]">
                                            </a>
                                            <!--{/loop}-->
                                            <div class="clear"></div>
                                        </div>
                                        ...
                                        {lang lastpost}:
                                        <span><!--{if $thread['lastposter']}--><a href="{if $thread[digest] != -2}home.php?mod=space&username=$thread[lastposterenc]{else}forum.php?mod=viewthread&tid=$thread[tid]&page={echo max(1, $thread[pages]);}{/if}" c="1">$thread[lastposter]</a><!--{else}-->$_G[setting][anonymoustext]<!--{/if}--></span><em class="deanatsign">@</em>
										$thread[lastpost]
										</i>
                                        ...
                                        <i class="y">
                                            ...
                                            <span class="dean_ftdate">$thread[dateline]</span>
                                        </i>
					...
					<!--{else}-->
						...
						<ul id="waterfall" class="ml waterfall cl deanwaterfall">
							<!--{loop $_G['forum_threadlist'] $key $thread}-->
							...
							<li>
								<!--{if !$_GET['archiveid'] && $_G['forum']['ismoderator']}-->
									<div style="position:absolute;margin:1px;padding:2px;background:#FFF;z-index:111;">
									<!--{if $thread['fid'] == $_G[fid]}-->
										<!--{if $thread['displayorder'] <= 3 || $_G['adminid'] == 1}-->
											<input onclick="tmodclick(this)" type="checkbox" name="moderate[]" value="$thread[tid]" />
										<!--{else}-->
								...
								<div class="waterfallpics">
                                    ...
                                    <div class="deanfenleilabel"><!--{if $thread[typehtml]}-->$thread[typehtml] $thread[sorthtml]<!--{/if}--></div>
								</div>
                                <div class="deanscbm">
                                    ...
                                    <div class="deanscbmtx">
                                    	<!--{if $thread['authorid'] && $thread['author']}-->
                                            <a href="home.php?mod=space&uid=$thread[authorid]">
                                                ...
                                                <!--{/if}-->
                                                <!--{if $thread['authorid'] && $thread['author']}-->
                                                <em>$thread[author]</em><!--{if !empty($verify[$thread['authorid']])}--> $verify[$thread[authorid]]<!--{/if}-->
                                                <!--{/if}-->
                                            </a>
                                ...
                                <div class="deanscbbs">
                                    <i class="deanscviews">$thread[views]</i>
                                    <i class="deansccomments">$thread[replies]</i>
                                    <i class="deansczans">$thread[recommends]</i>
						...
						<script type="text/javascript" src="$_G['style'][styleimgdir]/js/redef.js"></script>
